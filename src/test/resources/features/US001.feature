Feature: http://expedia.com test
  ...
  Scenario: User can search a roundtrip
    Given Flight options are shown and Roundtrip is selected as default
    And user Writes in “Flying from” text field the name of the <Bogota>
    And user Clicks on first suggestion for Bogota
    And Write in “Flying to” text field the name of the <San Francisco>
    And Click on first suggestion for San Francisco
    And Click on Departing field
    And Select March 2 2016 from calendar <03/02/2016>
    And Click on Returning field
    And Select March 2 2016 from calendar <03/12/2016>
    When Click on Search button
    Then “Flights-Search” service should response with a list of departure flights order by lowest price

    Scenario: User can sort by price and duration
      Given “Flights-Search” service should response with a list of departure flights order by lowest price
